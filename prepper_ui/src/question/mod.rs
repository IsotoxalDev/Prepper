        ...
        <div class="QuestionBox">
        ...
        </div>
            {
                question.with(|q| match &q.answer {
                    Answer::MCQ(ch) => view!{cx, <Choice choices=ch.choices() correct=vec![0]/>}.into_view(cx),
                    Answer::MSQ(ch) => view!{cx, <Choice choices=ch.choices() correct=ch.correct()/>}.into_view(cx),
                    ...
                    Answer::Formula(correct) => view!{cx, <Formula correct=correct.into()/>}.into_view(cx),
                })
        }
    }
}
